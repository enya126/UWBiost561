...
Implementation 5: Uses for loop to iterate over every node as a start node. For each node except the start node, temporarily add it to the candidate(c(start node)) and compute the edge density using edge_density_5(). Select the node that yields the highest edge density. Add that node only if the density remains ≥ alpha. Repeat this process until no nodes can be added without violating the density condition. Compare the final candidate and keep the best one. 

Implementation 10: It used a greedy heuristic helper function which has the same logic as implementation 5's function. It used a for loop to check each row of the matrix using a greedy heuristic helper function, compare the result clique, and return the best one. 
...
I think implementation 5's code was a little bit complicated with a repeat inside a for loop.
I think implementation 10's code was a little redundant with a for loop and a helper function inside the main function, and a helper function outside the main function.

But they all give good documentations and variable naming. 
...
test_that("Fails if input matrix is invalid", {
  ...
  diag(adj_invalid) <- 1
  expect_error(compute_maximal_partial_clique5(adj_invalid, 0.8))
})
...
test_that("compute_maximal_partial_clique returns a list with expected names", {
  ...
  result <- compute_maximal_partial_clique5(adj, alpha = 0.7)
...
test_that("Edge case: small graph with full clique", {
  ...
  result <- compute_maximal_partial_clique5(adj, alpha)
  clique <- result$clique_idx
...
test_that("Fails if input matrix is invalid", {
  adj_invalid <- matrix(1:9, nrow = 3)
  diag(adj_invalid) <- 1
  expect_error(compute_maximal_partial_clique10(adj_invalid, 0.8))
})

# check_output
test_that("compute_maximal_partial_clique returns a list with expected names", {
  adj <- generate_partial_clique(n = 10, clique_fraction = 0.5, clique_edge_density = 0.8)$adj_mat
  result <- compute_maximal_partial_clique10(adj, alpha = 0.7)
...
test_that("Edge case: small graph with full clique", {
  ...
  result <- compute_maximal_partial_clique10(adj, alpha)
  clique <- result$clique_idx
...
```{r}
set.seed(10)
data <- UWBiost561::generate_partial_clique(n = 80, 
                                            clique_fraction = 0.5,
                                            clique_edge_density = 0.95)
...
Simulation study: I am using generate_partial_clique() function to generate random graphs. I want to change n from 10 to 30 to generate symmetric adjacency matrices with different number of nodes. 
I am testing 15 different implementations of the function, which might include different methods like greedy heuristic, local search, and randomized approaches. 
I hope to learn which method could handle large matrices and proceed the result with less running time while returning the correct partial clique with edge density greater than alpha. 
I am doing 3 trials per level, so my maximum estimated simulation time would be 45 mins (`2*3*15*30`).
...
I talked with Kevin during office hours, and I decided to use my UWBiost561 package to do my final project. I will follow the instructions and create a website for it. 
...
Hi Kevin, I have a question regarding this HW4. I passed the devtools::check(). I ran simulation files locally, and it looks good. But I did not get any .Rout on the server by command sbatch HW4_simulation_execute.slurm. 

I was thinking of trials exceeding the time limitation of 30s, but I'm not sure if this is the issue or how to diagnose it.
Could you possibly help with this problem? 
Thank you so much and apologize for the late submission. 
