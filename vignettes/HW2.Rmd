...
```{r}
country_population <- country_population[country_population$year >= 1990 & country_population$year <= 2010,] %>%
  group_by(iso3c, country) %>%
  summarize(mean_population = as.integer(mean(population)))
country_population
```
...
```{r}
# filter & remove NAs
country_birth <- na.omit(country_birth[country_birth$year >= 1990 & country_birth$year <= 2010, c("iso3c", "year", "life_expect")])
# min max life expectancy
country_birth <- country_birth %>%
  group_by(iso3c) %>%
  summarize(min_life_expect = round(min(life_expect), 1), max_life_expect = round(max(life_expect),1))
# range
country_birth <- country_birth %>%
...
generate_randome_graph() generates adj_mat & rev_order. 
n is the input that defines the number of nodes in the adjacency matrix. 
clique_fraction and density_low defines the number of 0 and 1 in the matrix. 
rev_order: returns the idx of 1 appears in matrix and 0 appears in matrix. 
...
Add row number as a new column, then change previous rows into one column (value). Previous column names into a column (Y), and repeat previous Row values into another column (X). 
...
```
This is a diagonal symmetric heatmap that visualises the 0 and 1 in the adjacent matrix. 
...
```{r}
adj_mat2 <- pivot_longer_heatmap(adj_mat2)
ggplot(adj_mat2, aes(x = X, y = Y, fill = factor(Value))) +
  geom_tile() +  # Use geom_tile() to create a heatmap like plot
  scale_fill_manual(values = c("0" = "palegoldenrod", "1" = "coral2")) +
  coord_fixed(ratio = 1) # Set aspect ratio to 1:1
```
Adj_mat2 is changing both the rows and columns order, and it shows a cluster of 1s. This indicates there is also a cluster in the adj_mat, but it needs rearrangement to show the cluster. 
...
1. Give the function a sample input and check its output. 
2. Use some corner cases (very small matrix and very large matrix with many 1s or 0s) to test the function. 3. Use incorrect input to check the function's output.
4. check if this function run correctly when called in other projects. 
