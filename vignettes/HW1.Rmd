...
I took BIOST 522 and BIOST 523 which involved R programming, and I am pretty comfortable in coding in R.
...
I am expecting to learn more about writing R packages and deeper understanding in R coding instead of using functions from packages. Thank you Kevin!
...
Input is "n", output is "sample_vec". 
The function generate n numbers of 1,2 and 3 randomly. The total number of 1,2 and 3 decides the number generated from normal gamma or chisq distribution. The output is n generated numbers. 
...
data <- c()
for (i in c(1,2,3,5,10,100)) {
  for (j in 1:10000) {
  data[j] <- mean(generate_data(i))
  }
  hist(data, main = paste("Histogram", i), breaks = 100)
}
...
As the number of generated values increases, the distribution of the mean comes more towards a normal distribution, no matter what distribution the values originated from. 
...
head(df) display first parts of each variable in the data frame df. 
summary(df) provides a brief summary on each variable including length, class and summaries for quantitative variable.
...
hist(df$Age.at.Death, main = "Histogram of age at death")
```
...
summary(df)
```
It shows what factors are included in each variable with its length. Before, it only shows length of the variable.  
...
```{r}
table(Braak = df$Braak, CognitiveStatus = df$Cognitive.Status)
```
...
```{r}
breaks <- quantile(df$Last.CASI.Score, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
df$Last.CASI.Score_quartile <- cut(df$Last.CASI.Score, breaks = breaks, include.lowest = TRUE, labels = c("Q1", "Q2", "Q3", "Q4"))
table(df$Last.CASI.Score_quartile, df$Cognitive.Status)
```
More patients with CASI scores in the lower quantile has Dementia compared to patients with CASI scores in the upper quantile. 
